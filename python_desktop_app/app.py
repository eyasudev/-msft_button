...
def teams_api_start_call(): 
    ...
    print("CALL CREATED: Press the button to mute current call")
    while (True): 
        time.sleep(1)
        ...
        if prev_to_mute != to_mute: 
            ...
            print("CALL ACTIVE: muting? {}".format(to_mute))
            #this mutes the bot
                ...
                for participant in participant_data["value"]: 
                    try: 
                        if participant["info"]["identity"]["user"]["id"] == ret_data["user_id"]:
                            logging.info("!!Participant id found")
    ...
    while (True): 
        ...
        if not que.empty(): 
            ...
            if ret_pro["ret"] == 0: #good incoming call message
                ...
                print("CALL RECEIVED AND ANSWERED [Press the button to mute current call ]")
        ...
        if prev_to_mute != to_mute: 
            if notification_data["ret"] == 0: 
                ...
                print("ERROR! can't send cmd, No active call. Muting? {}".format(to_mute))
...
Thread_teams_api_start_call = threading.Thread(target=teams_api_start_call, args=( ))
...
if __name__ == "__main__": 
    ...
    if len(sys.argv) == 2 and sys.argv[1] == "--call": 
        Thread_teams_api_start_call.start()
    else :
